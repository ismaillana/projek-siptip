...
<div class="main-content">
    <section class="section">
      ...
      <form id="myForm" class="forms-sample" enctype="multipart/form-data" method="POST"
        action="{{route('evaluasi.update', $evaluasi) }}">
        @method('put')
        {{ csrf_field() }}
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-md-10">
                                    <select name="penugasan_id"
                                        class="form-control select2 @error('penugasan_id') is-invalid @enderror" disabled @readonly(true)>
                                        ...
                                        @foreach ($penugasan as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('penugasan_id', @$evaluasi->penugasan_id) == $item->id ? 'selected' : '' }}>
                                                {{ $item->tugas }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-md-10">
                                    <select name="status_jurnal"
                                        ...
                                        <option value="Revisi"
                                            {{ old('status', @$evaluasi->status) == 'Revisi' ? 'selected' : '' }}>
                                            Revisi</option>
                                        <option value="Selesai"
                                            {{ old('status', @$evaluasi->status) == 'Selesai' ? 'selected' : '' }}>
                                            Selesai</option>
                                    </select>
                                ...
                                <div class="col-md-10">
                                    <div class="input-group">
                                        <input class="dropify @error('file_revisi') is-invalid @enderror" type="file" 
                                            name="file_revisi" data-height='250' data-default-file="{{@$evaluasi->file_revisi_url}}" data-allowed-file-extensions="pdf" data-max-file-size="5M">
                                    </div>
